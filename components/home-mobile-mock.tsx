...
/**
 * The Figma mobile frame (1054×2360) rendered at mock scale: the whole
 * artboard scales as one object to the viewport width via {@link ZoomableStage},
 * so every tile, card and label keeps the exact proportion Ryan mocked, with
 * pinch-zoom 1×–3× as an opt-in gesture on top of the fit-to-width landing
 * state. Shared between `/` (mobile slot) and `/m` (the standalone mock
 * reference) so both routes render an identical mobile experience — the
 * reflowed-native-mobile approach was tried and rejected; this replaced it.
 */
export function HomeMobileMock() {
  return (
    <>
      {/* Stage height is cropped to 2128 — where the Figma frame's own
          `mobile_buttons` band starts (frame is 2360 tall) — matching how
          /about and /methodology crop under their own mobile_buttons bar.
          The real bottom bar is the fixed MobileNavBar below. */}
      <ZoomableStage width={1054} height={2128}>
        <Lattice marks={marks} />
